...
/**
 * Servlet implementation class searchngo
 */
@WebServlet("/searchngo")
public class searchngo extends HttpServlet {
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try
		{
			...
			s.executeUpdate("TRUNCATE table ngotblscore");
			...
			if(rss.next())
			{
			 ...
			 String query="select * from user_product_info where name LIKE ?";
			...
			if(rs1.next())
			{
		       ...
		       // if(distance>=th)
		        {
		        String sql="insert into ngotblscore(From_City,To_City,Product_Name,Km_Location,F_ID) values(?,?,?,?,?)";
		...
		RequestDispatcher rd = request.getRequestDispatcher("/NGO_ShowProduct.jsp");
