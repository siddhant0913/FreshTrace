...
//			d2 = format.parse(end);
	...
	protected void doPost(HttpServletRequest request, HttpServletResponse response) throws ServletException, IOException {
		...
		try {
			...
			ResultSet rs1=st1.executeQuery("select * from product_info where name='"+name+"' and farmeremail='"+farmeremail+"'");
			if(rs1.next())
			{
				flag=1;
			...
			ResultSet rs=st.executeQuery("select max(ID) from product_info ");
			if(rs.next())
			{
			...
			PreparedStatement ps=con.prepareStatement("insert into product_info(name,qty,product_description,product_price,Location_Food,exp_date,farmeremail) values(?,?,?,?,?,?,?)");
			ps.setString(1, name);
			ps.setString(2, qty);
